 ...
 * @returns 密文或报错
 */
const encrypt = (content: any, isReferenceDataType: Boolean = false): string | undefined | never => {
...
const decrypt = (secret: string, isReferenceDataType: Boolean = false): any => {
    let bytes = CryptoJS.AES.decrypt(secret, secret_key)
    let originContent = bytes.toString(CryptoJS.enc.Utf8)
